...
const NODE_NPM_MAPPING = {
    '4.0.0': '2.14.2', '6.0.0': '3.8.6', '8.0.0': '5.0.0',
    '10.0.0': '6.0.0', '12.0.0': '6.9.0', '14.0.0': '6.14.4',
    '16.0.0': '7.10.0', '18.0.0': '8.6.0', '20.0.0': '9.6.4',
    '21.0.0': '10.2.0', '22.0.0': '10.4.0'
}
...
const PACKAGE_NODE_REQUIREMENTS = {
    ...
    'next': { '10.0.0': '10.13.0', '11.0.0': '12.0.0', '12.0.0': '12.22.0', '13.0.0': '16.14.0', '14.0.0': '18.17.0' },
    'express': { '4.0.0': '0.10.0', '5.0.0': '12.0.0' },
    'commander': { '10.0.0': '14.0.0', '11.0.0': '16.0.0', '12.0.0': '16.0.0', '13.0.0': '16.0.0', '14.0.0': '18.0.0' },
...
function getNodeRequirementForPackage(packageName, packageVersion) {
    ...
    const versions = Object.keys(requirements).sort((a, b) => compareVersions(a, b))
...
async function collectVersionConstraints(quiet = false) {
    ...
    if (fs.existsSync(packageJsonPath)) {
        ...
        if (packageJsonData) {
            if (!quiet) console.log(`Project ${packageJsonData.name} analyzing dependencies...`)

            // Add engine constraints if present
            if (packageJsonData.nodeVersion) {
                if (!quiet) console.log(`Found Node.js engine requirement: ${packageJsonData.nodeVersion}`)
                constraints.push({
            ...
            for (const [name, version] of Object.entries(packageJsonData.dependencies)) {
                const nodeRequirement = getNodeRequirementForPackage(name, version)
                if (nodeRequirement) {
                    if (!quiet) console.log(`Dependency ${name}@${version} requires Node.js ${nodeRequirement}`)
                    constraints.push({
                        name: name,
            ...
            for (const [name, version] of Object.entries(packageJsonData.devDependencies)) {
                const nodeRequirement = getNodeRequirementForPackage(name, version)
                if (nodeRequirement) {
                    if (!quiet) console.log(`DevDependency ${name}@${version} requires Node.js ${nodeRequirement}`)
                    constraints.push({
                        name: name,
                        nodeVersion: nodeRequirement,
    ...
    if (fs.existsSync(lockfilePath)) {
        if (!quiet) console.log('Found package-lock.json, analyzing dependency tree...')
        try {
            ...
            if (lockfileContent.packages) {
                for (const [pkgPath, pkgInfo] of Object.entries(lockfileContent.packages)) {
                    ...
                    if (pkgInfo.version) {
                        const nodeRequirement = getNodeRequirementForPackage(pkgName, pkgInfo.version)
                        if (nodeRequirement) {
                            if (!quiet) console.log(`Dependency ${pkgName}@${pkgInfo.version} requires Node.js ${nodeRequirement}`)
                            constraints.push({
                                name: pkgName,
                                nodeVersion: nodeRequirement,
                                source: 'lockfile'
            ...
            // For older npm lockfile formats (npm v6 and below)
                ...
                for (const [pkgName, pkgInfo] of Object.entries(lockfileContent.dependencies)) {
                    if (pkgInfo.version) {
                        const nodeRequirement = getNodeRequirementForPackage(pkgName, pkgInfo.version)
                        if (nodeRequirement) {
                            if (!quiet) console.log(`Dependency ${pkgName}@${pkgInfo.version} requires Node.js ${nodeRequirement}`)
            ...
            if (!quiet) console.warn(`Warning: Could not parse package-lock.json: ${error.message}`)
        }
...
function getVersionSummary(constraints) {
    ...
    const engineConstraints = constraints.filter(c => c.source === 'engines' && (c.nodeVersion || c.npmVersion))
    const dependencyConstraints = constraints.filter(c => (c.source === 'dependency' || c.source === 'devDependency' || c.source === 'lockfile') && c.nodeVersion)
    ...
    for (const constraint of allConstraints) {
        ...
        if (constraint.nodeVersion) {
            ...
            if (lowestVersion) {
                ...
                if (normalizedVersion) {
                    ...
                    if (!lowestNodeVersion || compareVersions(normalizedVersion, lowestNodeVersion) > 0) {
                        lowestNodeVersion = normalizedVersion
                        // Track which type of constraint provided the final minimum version
                        sourceType = constraint.source === 'engines' ? 'engines' : 'dependencies'
                    }

                    // Check for upper bounds
                    const highestVersion = getHighestCompatibleVersion(constraint.nodeVersion)
                    if (highestVersion) {
                        ...
                        if (normalizedHighest) {
                            ...
                            if (!highestNodeVersion || compareVersions(normalizedHighest, highestNodeVersion) < 0) {
        ...
        if (constraint.npmVersion && constraint.source === 'engines') {
            ...
            if (lowestNpmVer) {
                ...
                if (normalizedNpmVer) {
                    if (!lowestNpmVersion || compareVersions(normalizedNpmVer, lowestNpmVersion) > 0) {
            ...
            if (highestNpmVer) {
                ...
                if (normalizedHighestNpm) {
                    if (!highestNpmVersion || compareVersions(normalizedHighestNpm, highestNpmVersion) < 0) {
    ...
    return {
        lowest: {
            ...
            npm: lowestNpmVersion || (lowestNodeVersion ? getNpmVersionForNode(lowestNodeVersion) : null)
        },
        highest: {
            node: highestNodeVersion || 'unlimited',
            npm: highestNpmVersion || (highestNodeVersion ? getNpmVersionForNode(highestNodeVersion) : 'latest compatible')
        },
        source: sourceType
    }
}
